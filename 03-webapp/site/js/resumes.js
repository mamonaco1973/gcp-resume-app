...
export function bindResumeHandlers() {
  ...
  newButton?.addEventListener("click",  () => resetResumeForm());
  closeButton?.addEventListener("click", () => closeResumeManager());
  form?.addEventListener("submit", async (event) => {
    event.preventDefault();
    await saveResume();
  });
  deleteButton?.addEventListener("click", async () => handleDeleteResume());
...
async function saveResume() {
  ...
  if (!resumeName) { window.alert("Resume name is required.");  return; }
  if (!resumeText) { window.alert("Resume text is required.");  return; }

  try {
    if (saveButton) {
      saveButton.disabled    = true;
      saveButton.textContent = selectedResumeId ? "Updating..." : "Creating...";
    }

    if (selectedResumeId) {
      await updateResume(selectedResumeId, { name: resumeName, resume: resumeText });
    } else {
      await createResume({ name: resumeName, resume: resumeText });
    }
    ...
    if (selectedResumeId) {
      ...
      const match = resumes.find((r) => r.name === resumeName);
      if (match) await selectResume(match.resume_id);
      else resetResumeForm();
    }
  } catch (error) {
    ...
    if (saveButton) {
      ...
      saveButton.textContent = selectedResumeId ? "Update Resume" : "Create Resume";
    }
  }
}
...
/* Purpose: Confirm deletion, call the API, clear selection tracking, and     */
/*          reset the form after a successful delete.                          */
/* -------------------------------------------------------------------------- */
async function handleDeleteResume() {
  ...
  try {
    ...
    if (lastSelectedResumeId === resumeIdToDelete) lastSelectedResumeId = null;
    await refreshResumeList();
    resetResumeForm();
  } catch (error) {
...
function renderResumeList() {
  ...
  resumes.forEach((resume) => {
    ...
    item.innerHTML = `
      <div class="resume-list-name">${escapeHtml(resume.name || "Untitled Resume")}</div>
      <div class="resume-list-meta">${formatDate(resume.updated_at || resume.created_at)}</div>
    `;
    item.addEventListener("click", async () => selectResume(resume.resume_id));
