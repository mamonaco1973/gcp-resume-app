...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
  "https://us-central1-aiplatform.googleapis.com/v1/projects/${PROJECT_ID}/locations/us-central1/publishers/google/models/${GEMINI_MODEL_ID}:generateContent" \
...
if [ "${HTTP_CODE}" != "200" ]; then
  ...
  echo "       Ensure the Vertex AI API is enabled and the SA has aiplatform.user role."
  exit 1
fi
